...
const HRMSCalendar = () => {
	...
	const fetchAttendanceData = async (startDate, endDate) => {
		try {
			...
			return user.role === "Admin"
				? response.data
				: response.data.filter((a) => a.employeeId === user.employeeId);
		} catch (error) {
	...
	const fetchLeaveData = async (startDate, endDate) => {
		try {
			...
			return user.role === "Admin"
				? response.data
				: response.data.filter((l) => l.employeeId === user.employeeId);
		} catch (error) {
	...
	const processAttendanceEvents = (attendanceData) => {
		return attendanceData.map((attendance) => ({
			...
			title: `${attendance.status} - ${
				attendance.employee?.firstName || "Employee"
			}`,
	...
	const EventComponent = ({ event }) => {
		...
		return (
			<div className="text-xs">
				...
				{resource.type === "leave" && (
					<div className="text-xs opacity-90 capitalize">{resource.status}</div>
				)}
			</div>
		);
	};
